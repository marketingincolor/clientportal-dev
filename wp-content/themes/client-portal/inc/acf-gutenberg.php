 ...
 */
function client_portal_acf_init() {
	...
	acf_register_block_type(
		array(
			'name'            => 'wds-accordion',
			'title'           => esc_html__( 'Accordion', 'client-portal' ),
			'description'     => esc_html__( 'A custom set of collapsable accordion items.', 'client-portal' ),
			'render_callback' => 'client_portal_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'sort',
			'keywords'        => array( 'accordion', 'wds' ),
			'mode'            => 'preview',
			'enqueue_assets'  => 'client_portal_acf_enqueue_accordion_scripts',
			'align'           => 'wide',
			'supports'        => $supports,
			'example'         => array(
				'attributes' => array(
					'data' => array(
						'title'           => esc_html__( 'Accordion Block Title', 'client-portal' ),
						'text'            => esc_html__( 'Accordion Block Content', 'client-portal' ),
						'accordion_items' => array(
							'0' => array(
								'accordion_title' => esc_html__( 'Accordion Item Title', 'client-portal' ),
								'accordion_text'  => esc_html__( 'Accordion Item Content', 'client-portal' ),
							),
						),
					),
				),
	...
	acf_register_block_type(
		array(
			'name'            => 'wds-carousel',
			'title'           => esc_html__( 'Carousel', 'client-portal' ),
			'description'     => esc_html__( 'A carousel with a call to action for each slide.', 'client-portal' ),
			'render_callback' => 'client_portal_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'slides',
			'keywords'        => array( 'carousel', 'slider', 'wds' ),
			'mode'            => 'preview',
			'enqueue_assets'  => 'client_portal_acf_enqueue_carousel_scripts',
			'align'           => 'wide',
			'supports'        => $supports,
			'example'         => array(
				'attributes' => array(
	...
	acf_register_block_type(
		array(
			'name'            => 'wds-cta',
			'title'           => esc_html__( 'Call To Action', 'client-portal' ),
			'description'     => esc_html__( 'A call to action block.', 'client-portal' ),
			'render_callback' => 'client_portal_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'megaphone',
			'keywords'        => array( 'call to action', 'cta', 'wds' ),
			'mode'            => 'preview',
			'enqueue_assets'  => 'client_portal_acf_enqueue_backend_block_styles',
			'align'           => 'wide',
			'supports'        => $supports,
			'example'         => array(
				'attributes' => array(
					'data' => array(
						'title'       => esc_html__( 'Call To Action Title', 'client-portal' ),
						'text'        => esc_html__( 'Call To Action Text', 'client-portal' ),
						'button_link' => array(
							'title' => esc_html__( 'Learn More', 'client-portal' ),
							'url'   => '#',
						),
					),
				),
	...
	acf_register_block_type(
		array(
			...
			'title'           => esc_html__( 'Fifty/Fifty Block', 'client-portal' ),
			'description'     => esc_html__( 'A split-content block with text and/or media.', 'client-portal' ),
			'render_callback' => 'client_portal_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'editor-table',
			...
			'example'         => array(
				'attributes' => array(
					'data' => array(
						'text_primary' => esc_html__( 'Primary Text content', 'client-portal' ),
						'media_right'  => array(
	...
	acf_register_block_type(
		array(
			'name'            => 'wds-hero',
			'title'           => esc_html__( 'Hero Block', 'client-portal' ),
			'description'     => esc_html__( 'A hero with an optional call to action.', 'client-portal' ),
			'render_callback' => 'client_portal_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'slides',
	...
	acf_register_block_type(
		array(
			...
			'title'           => esc_html__( 'Recent Posts Block', 'client-portal' ),
			'description'     => esc_html__( 'A set of recent posts displayed by category and/or tag.', 'client-portal' ),
			'render_callback' => 'client_portal_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'admin-page',
	...
	acf_register_block_type(
		array(
			...
			'title'           => esc_html__( 'Related Posts Block', 'client-portal' ),
			'description'     => esc_html__( 'A set of manually selected posts.', 'client-portal' ),
			'render_callback' => 'client_portal_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'admin-page',
...
add_action( 'acf/init', 'client_portal_acf_init' );
 ...
 * @author Corey Collins
 */
function client_portal_acf_block_registration_callback( $block ) {
	...
	if ( ! is_admin() && client_portal_has_block_expired(
		array(
	...
	client_portal_display_expired_block_message();
 ...
 * @author Corey Collins
 */
function client_portal_add_block_categories( $categories, $post ) {
	...
	return array_merge(
		...
		array(
			array(
				'slug'  => 'wds-blocks',
				'title' => esc_html__( 'WDS Blocks', 'client-portal' ),
			),
		)
	);
}
add_filter( 'block_categories', 'client_portal_add_block_categories', 10, 2 );
 ...
 * @author Corey Collins
 */
function client_portal_acf_enqueue_backend_block_styles() {
 ...
 * @author Corey Collins
 */
function client_portal_acf_enqueue_carousel_scripts() {

	if ( ! is_admin() ) {
		return;
	}

	client_portal_acf_enqueue_backend_block_styles();
 ...
 * @author Corey Collins
 */
function client_portal_acf_enqueue_accordion_scripts() {
 ...
 * @author Corey Collins
 */
function client_portal_get_block_alignment( $block ) {
 ...
 * @author Corey Collins
 */
function client_portal_get_block_classes( $block ) {
	...
	$classes  = '';
	$classes  = client_portal_get_block_expired_class();
 ...
 * @author Corey Collins
 */
function client_portal_get_block_expired_class() {
	...
	if ( client_portal_has_block_expired(
		array(
 ...
 * @author Corey Collins
 */
function client_portal_display_expired_block_message() {

	if ( ! client_portal_get_block_expired_class() ) {
		return;
	}

	?>
	<div class="block-expired-message">
		<span class="block-expired-text"><?php esc_html_e( 'Your block has expired. Please change or remove the Start and End dates under Other Options to display your block on the frontend.', 'client-portal' ); ?></span>
	</div>
	<?php
}
 ...
 * @author Corey Collins
 */
function client_portal_get_block_id( $block ) {
 ...
 * @author Corey Collins
 */
function client_portal_acf_gutenberg_display_admin_default_carousel( $block ) {
	...
	for ( $slides = 0; $slides < 2; $slides++ ) :
		...
		<section class="slide">
			<div class="slide-content container">
				<h2 class="slide-title"><?php esc_html_e( 'Slide Title', 'client-portal' ); ?></h2>
				<p class="slide-description"><?php esc_html_e( 'Slide Content', 'client-portal' ); ?></p>
			</div>
		</section>
	<?php
	endfor;
