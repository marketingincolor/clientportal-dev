 ...
 */
function client_portal_get_attachment_id_from_url( $attachment_url = '' ) {
 ...
 * @return string
 */
function client_portal_get_placeholder_unsplash( $args = array() ) {
	...
	if ( ! is_numeric( $args['width'] ) ) {
		throw new Exception( esc_html__( 'Width must be an integer', 'client-portal' ) );
	}

	if ( ! is_numeric( $args['height'] ) ) {
		throw new Exception( esc_html__( 'Height must be an integer', 'client-portal' ) );
	}
	...
	$alt = apply_filters( 'client_portal_placeholder_image_alt', esc_html__( 'WebDevStudios Placeholder Image', 'client-portal' ) );
 ...
 * @return string
 */
function client_portal_copyright_year( $atts ) {
...
add_shortcode( 'client_portal_copyright_year', 'client_portal_copyright_year', 15 );
...
if ( defined( 'WPSEO_VERSION' ) ) {
	...
	add_filter( 'wpseo_metabox_prio', 'client_portal_move_yoast_to_bottom' );
}
 ...
 * @author jomurgel
 */
function client_portal_get_the_content( $content ) {
...
add_filter( 'the_content', 'client_portal_get_the_content', 20 );
