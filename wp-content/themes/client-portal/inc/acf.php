 ...
 */
function client_portal_display_block_options( $args = array() ) {
	...
	// Get the block ID.
	$block_id = client_portal_get_block_id( $args['block'] );
	...
	$has_show_overlay = client_portal_has_array_key( 'show_overlay', $background_options ) && true === $background_options['show_overlay'] ? ' has-overlay' : '';
	...
	if ( $args['background_type'] ) {
		...
		if ( 'video' === $args['background_type'] ) {
			...
			$background_alt = $background_title ? sprintf( esc_attr( 'Video Background of %s', 'client-portal' ), esc_attr( $background_options['background_video_title'] ) ) : __( 'Video Background', 'client-portal' );
			...
			?>
				...
				<button class="video-toggle"><span class="screen-reader-text"><?php esc_html_e( 'Toggle video playback', 'client-portal' ); ?></span></button>
			<?php
 ...
 * @return bool
 */
function client_portal_has_block_expired( $args = array() ) {
 ...
 */
function client_portal_acf_flexible_content_layout_title( $block_title, $field, $layout, $i ) {
	...
	if ( 'none' !== $background ) {
		...
		$type = client_portal_return_flexible_content_layout_value( $background_type );
		...
		if ( 'video' === $background_type ) {
			$block_heading .= '<div style="font-size: 30px; height: 26px; width: 30px;" class="dashicons dashicons-format-video acf-flexible-title-image"><span class="screen-reader-text">' . esc_html__( 'Video', 'client-portal' ) . '</span></div>';
		}
	}
	...
	if ( client_portal_has_block_expired(
		...
		$expired .= '<span style="color: red;">&nbsp;(' . esc_html__( 'expired', 'client-portal' ) . ')</span>';
	}
...
add_filter( 'acf/fields/flexible_content/layout_title/name=content_blocks', 'client_portal_acf_flexible_content_layout_title', 10, 4 );
 ...
 */
function client_portal_return_flexible_content_layout_value( $type ) {
...
if ( function_exists( 'client_portal_acf_flexible_content_layout_title' ) ) {

	/**
	 * Set Admin Styles for Flexible Content Layout Image/Title in client_portal_acf_flexible_content_layout_title().
	 *
	 * @author WDS
	 */
	function client_portal_flexible_content_layout_title_acf_admin_head() {
	?>
	<style type="text/css">
	...
	add_action( 'acf/input/admin_head', 'client_portal_flexible_content_layout_title_acf_admin_head' );
}

/**
 * Load colors dynamically into select field from client_portal_get_theme_colors()
 *
 * @author WDS
 ...
 */
function client_portal_acf_load_color_picker_field_choices( $field ) {
...
add_filter( 'acf/load_field/name=color_picker', 'client_portal_acf_load_color_picker_field_choices' );
 ...
 */
function client_portal_get_theme_colors() {
	return array(
		esc_html__( 'Alto', 'client-portal' )           => '#ddd',
		esc_html__( 'Black', 'client-portal' )          => '#000',
		esc_html__( 'Blue', 'client-portal' )           => '#21759b',
		esc_html__( 'Cod Gray', 'client-portal' )       => '#111',
		esc_html__( 'Dove Gray', 'client-portal' )      => '#666',
		esc_html__( 'Gallery', 'client-portal' )        => '#eee',
		esc_html__( 'Gray', 'client-portal' )           => '#808080',
		esc_html__( 'Gray Alt', 'client-portal' )       => '#929292',
		esc_html__( 'Light Yellow', 'client-portal' )   => '#fff9c0',
		esc_html__( 'Mineshaft', 'client-portal' )      => '#333',
		esc_html__( 'Silver', 'client-portal' )         => '#ccc',
		esc_html__( 'Silver Chalice', 'client-portal' ) => '#aaa',
		esc_html__( 'White', 'client-portal' )          => '#fff',
		esc_html__( 'Whitesmoke', 'client-portal' )     => '#f1f1f1',
	);
}
 ...
 * @return void
 */
function client_portal_display_hero_heading( $block_title ) {
 ...
 * @since NEXT
 */
function client_portal_display_link( $args = array() ) {
	echo client_portal_get_link( $args ); // WPCS: XSS Ok.
}
 ...
 */
function client_portal_get_link( $args = array() ) {
	...
	$title = client_portal_has_array_key( 'title', $button_array ) ? $button_array['title'] : esc_html__( 'Read More', 'client-portal' );

	// Get url.
	$url = client_portal_has_array_key( 'url', $button_array ) ? $button_array['url'] : '';

	// Get target, else default internal.
	$target = client_portal_has_array_key( 'target', $button_array ) ? $button_array['target'] : '_self';
	?>
